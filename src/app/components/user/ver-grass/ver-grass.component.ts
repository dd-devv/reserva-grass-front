...
export class VerGrassComponent implements OnInit {
  ...
  public socket = io('https://api.reservatugrass.com', { path: '/socket.io' });
      ...
      if (this.horasReserva > 4 || this.horasReserva < 1) {
        ...
        if (boton.disponible) {
          ...
          let data = {
            ...
            subtotal: this.horasReserva * this.cancha_ver.precio_reservacion,
          ...
          this._userService.crear_reservacion_user(data, this.token).subscribe({
            ...
            error: (err) => {
              this._toastrService.error(err.error.message || 'No se pudo registrar, intete de nuevo', 'ERROR');
  ...
  select_mas_una_hora(hora: string) {
    // Convertir el valor de hora a un número entero
    this.horasInicio = parseInt(hora);

    // Asegurarse de que horasReserva no supere el máximo permitido de 4
    const horasReservaValidas = Math.min(this.horasReserva, 4);

    // Calcular la hora final basándose en el valor validado de horasReserva
    this.horasFinal = this.horasInicio + horasReservaValidas;

    // Determinar si se seleccionaron más de una hora
